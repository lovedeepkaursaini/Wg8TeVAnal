...
class analyze {
 public :
  ...
  analyze(TString filename_,TTree *tree=0);
   TString filenam;
  virtual ~analyze();
  ...
  virtual bool     eIDTight2012(int i);
  virtual bool     eIDLoose2012(int i);
  virtual bool     eIDVeto2012(int i);
  virtual bool     eIDMedium2012(int i);
  ...
  virtual float SF_tightPhoID(float pt, float eta);
  virtual float SF_mediumPhoID_EleVeto(float pt, float eta);
  virtual float SF_TightEID(float pt, float eta);
  virtual float SF_LooseEID(float pt, float eta);
  virtual float SF_MediumEID(float pt, float eta);
  virtual float PixelSeedVeto(float pt, float eta);
  virtual bool HasMatchingGSFelectron(vector<int> ielectrons, int ipho);
...
#ifdef analyze_cxx
analyze::analyze(TString filenam_,TTree *tree) : fChain(0) 
{
  //TChain* chain = new TChain("EventTree");
  //chain->Add("../Condor/job_summer12_Wg_skimmed_wg.root");
  //tree = chain;
 // Init(tree);

  if (tree == 0) {
    filenam = filenam_;
    TString nam = "/eos/uscms/store/user/lovedeep/Wg/"+filenam_+".root";
