...
float lepton1Eta=0;
float lepton1Phi=0;
float lepton1SCEta=0;
float lepton1SCPhi=0;
float lepton1RelIso=0;
float lepton1Pt=0;
float lepton2Pt=0;
float lepton2Eta=0;
int run=-999 ;
...
void analyze::Loop(TString name)
{
  ...
  */
  ...
  miniTree->Branch("genPartPdgId",&genPartPdgId);
  miniTree->Branch("ilep1", &ilep1);
  miniTree->Branch("boolVetoID", &boolVetoID);
  ...
  miniTree->Branch("MatchGenPho", &MatchGenPho, "MatchGenPho/F");
  miniTree->Branch("MatchGenEle", &MatchGenEle, "MatchGenEle/F");
...
//    if(ielectrons.size()>1 && eIDVeto2012(ielectrons[1]) ) continue;
        hCounter->Fill(13);
...
lepton1RelIso = ((*elePFChIso03)[i] + TMath::Max((Double_t) 0., (Double_t) (*elePFPhoIso03)[i] + (*elePFNeuIso03)[i] - EffectiveArea*rho2012));
if(ielectrons.size()>1) lepton2Pt = elePt->at(ielectrons[1]);
if(ielectrons.size()>1) lepton2Eta = eleEta->at(ielectrons[1]);
if(ielectrons.size()>1) boolVetoID = eIDVeto2012(ielectrons[1]);
...
//cout<<"kne cuts pass"<<endl;  
//Sep15,15...check with tight ID
...
//cout<<(*phoEt)[ipho]<<'\t'<<(*phoEta)[ipho]<<'\t'<<(*phoSCEta)[ipho]<<'\t'<<(*phoPhi)[ipho]<<endl;
//cout<<pho.DeltaR(ele)<<endl;
      if( pho.DeltaR(ele) < 0.7 ) continue;
      //    pre_photons.push_back(ipho);
//      cout<<"dRpass"<<endl;
...
double dr2 = 0;      
      ...
      for (int genPho = 0; genPho < nMC; ++genPho){
        ...
        if( ((*mcParentage)[genPho]& 4) == 4) continue;
        if( (*mcPt)[genPho] < 10 ) continue;
        if((fabs((*phoEt)[j] - (*mcPt)[genPho]) / (*mcPt)[genPho]) > 0.1)continue;
        if((fabs(pho_eta-(*mcEta)[genPho]))>0.005) continue;
        dr2 = dR(pho_eta,pho.Phi(),(*mcEta)[genPho],(*mcPhi)[genPho]);
        if(dr2< dr_min2) dr_min2=dr2;
        genPhoPt = pho.Pt();
        }// end genP loop 
 ...
 int matchingGenInd = -9;
      for (int genPho = 0; genPho < nMC; ++genPho){
        ...
        dr3 = dR(pho_eta,pho.Phi(),(*mcEta)[genPho],(*mcPhi)[genPho]);
         // CALCULATE DR
        if(dr3 < dr_min3){dr_min3 = dr3; matchingGenInd = genPho;}
         }
        
        MatchGenEle = dr3;
        if(MatchGenEle<0.3) h_genPartPdgId->Fill(fabs((*mcPID)[matchingGenInd]), totalWeight);
        genPartPdgId = (*mcPID)[matchingGenInd];
}	
...
int main(int argc, char* argv[]) {
  ...
  t.Loop("miniTreewSFsNoSieieIsoMegPSVwMt_"+fName+".root");
